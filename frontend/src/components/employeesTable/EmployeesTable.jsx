...
import { getEmployeeProfile } from '../../context/actions/employeeActions/getEmployeeProfile';
...
const EmployeesTable = () => {
    ...
    const { data, isLoading, isError, error, isSuccess } = employeeTableState;

    // console.log("checkGetEmployeeState", data, isLoading, isError, error, isSuccess)
    ...
    const handlePageChange = (event, newPage) => {
        ...
        getEmployeeTable({ search, gender, status, sort, page: newPage })(employeeTableDispatch)
    ...
    const handleEditClick = (e, id) => {
        setTableRowId(id);
        getEmployeeProfile({ id })(employeeTableDispatch)
    ...
    const handleSearch = (searchedVal) => {
        ...
        setCurrentPage(1);
        if (searchedVal == "") {
            getEmployeeTable({ search: searchedVal, gender, status, sort, page: 1 })(employeeTableDispatch)
        }
    }

    const requestSearch = () => {
        setCurrentPage(1);
        getEmployeeTable({ search, gender, status, sort, page: 1 })(employeeTableDispatch)
    };

    const handleGenderChange = (event) => {
        setGender(event.target.value);
        setCurrentPage(1);
        getEmployeeTable({ search, gender: event.target.value, status, sort, page: 1 })(employeeTableDispatch)
    };

    const handleStatusChange = (event) => {
        setStatus(event.target.value);
        setCurrentPage(1);
        getEmployeeTable({ search, gender, status: event.target.value, sort, page: 1 })(employeeTableDispatch)
    };

    const handleSortChange = (event) => {
        setSort(event.target.value);
        setCurrentPage(1);
        getEmployeeTable({ search, gender, status, sort: event.target.value, page: 1 })(employeeTableDispatch)
    };



    useEffect(() => {
        getEmployeeTable({ search, gender, status, sort, page: currentPage })(employeeTableDispatch)
